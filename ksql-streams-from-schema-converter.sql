-- BLOB_PARSE_T24
-- parse blob stream
CREATE STREAM FBNK_${procName}_MAPPED WITH (KAFKA_TOPIC='FBNK_${procName}_MAPPED', PARTITIONS=3) AS SELECT
	DATA.ROWKEY ROWKEY, 
  ...
  PARSE_T24_BLOB(DATA.XMLRECORD,'${blobDelim}' ,'MAP_SS_TEST_FBNK_${schemaName}-value', '#') XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#
...
CREATE STREAM FBNK_${procName}_MAPPED WITH (KAFKA_TOPIC='FBNK_${procName}_MAPPED', PARTITIONS=3) AS SELECT
	...
	DATA.SCN COMMIT_SCN,  
	DATA.OP_TYPE COMMIT_ACTION,  
  SPLIT_BLOB(DATA.XMLRECORD, 'FE', false) XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#
FROM FBNK_${procName} DATA
EMIT CHANGES;
...
CREATE OR REPLACE STREAM FBNK_${procName}_MULTIVALUE AS SELECT
  ...
  ${listSelectedField}
  PARSE_T24_MULTIVAL(DATA.RECID, DATA.XMLRECORD, 'FBNK_${procName}_MULTIVALUE', ARRAY[${vm}], ARRAY[${vs}], '#') XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#
...
CREATE OR REPLACE STREAM DW_${procName} WITH (PARTITIONS=3) AS SELECT
  ...
  DATA.COMMIT_ACTION COMMIT_ACTION,
      ${selectedFields}
  CAST(DATA.XMLRECORD->IDX AS INTEGER) V_M,
  CAST('1' AS INTEGER) V_S,
  (CASE WHEN ((DATA.XMLRECORD->IDX = '0') OR (DATA.COMMIT_ACTION = 'D')) THEN 'D' ELSE 'LIVE' END) FLAG_STATUS,
  (CASE WHEN ((SCP.IS_COB_COMPLETED = true) AND (CAST(DATA.COMMIT_SCN AS BIGINT) > SCP.COMMIT_SCN)) THEN PARSE_DATE(SCP.TODAY, 'yyyyMMdd') ELSE PARSE_DATE(SCP.LAST_WORKING_DAY, 'yyyyMMdd') END) BANKING_DATE
FROM FBNK_${procName}_MULTIVALUE DATA
INNER JOIN FBNK_SEAB_COB_PROCESS_3_PART SCP ON ((SCP.RECID = DATA.LOOKUP_KEY)) --Team SB kiểm tra lại topic cần join --
PARTITION BY DATA.ROWKEY
EMIT CHANGES;
...
CREATE OR REPLACE STREAM FBNK_${procName}_MAPPED  AS SELECT
  ...
  DATA.SCN COMMIT_SCN,  
  DATA.OP_TYPE COMMIT_ACTION, 
  PARSE_T24_RECORD(DATA.XMLRECORD, 'MAP_SS_TEST_FBNK_${schemaName}-value', '#') XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#
FROM FBNK_${procName}  DATA
EMIT CHANGES;
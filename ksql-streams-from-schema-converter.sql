-- BLOB
-- parse blob stream
CREATE OR REPLACE STREAM ${schemaName}_MAPPED AS
SELECT
...
-- RAW
-- raw/ogg stream
CREATE OR REPLACE STREAM ${schemaName} (
    ROWKEY STRING KEY,
    LOOKUP_KEY STRING,
    RECID STRING,
    `TABLE` STRING,
    ...
    XID STRING,
    XMLRECORD STRING)
WITH (FORMAT='avro', KAFKA_TOPIC='${schemaName}');
...
FROM
    ${sourceStream} DATA
    INNER JOIN SEAB_COB_PROCESS_3_PART SCP ON (SCP.RECID = DATA.LOOKUP_KEY)
...
-- XML
-- parse xml stream 
CREATE OR REPLACE STREAM ${schemaName}_MAPPED WITH (KAFKA_TOPIC='${schemaName}_MAPPED', PARTITIONS=3) AS SELECT
  DATA.ROWKEY ROWKEY,
  DATA.LOOKUP_KEY LOOKUP_KEY,
  DATA.RECID RECID,  
  DATA.OP_TS OP_TS,  
  ...
  DATA.SCN COMMIT_SCN,  
  DATA.OP_TYPE COMMIT_ACTION, 
  PARSE_T24_RECORD(DATA.XMLRECORD, 'MAP_SS_${schemaName}-value', '#') XMLRECORD --chọn delimiter nào ko xuất hiện trong value gốc VD:#
